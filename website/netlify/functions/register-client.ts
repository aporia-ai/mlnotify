import { Handler } from '@netlify/functions'
import * as admin from 'firebase-admin'

const handler: Handler = async event => {
	// Parse request
	let registrationToken: string, topicId: string
	try {
		const body: { registrationToken?: string; topicId?: string } = JSON.parse(event.body)
		if (!body.registrationToken || !body.topicId) throw new Error('Missing params')

		registrationToken = body.registrationToken
		topicId = body.topicId
	} catch (e) {
		return {
			statusCode: 400,
			body: JSON.stringify({
				message: 'Invalid input. Body must be of type { registrationToken: string, topicId: string }',
				error: e.message,
			}),
		}
	}

	// Initialize Firebase and subscribe to topic
	admin.initializeApp(
		{
			credential: admin.credential.cert({
				// @ts-ignore
				type: 'service_account',
				project_id: 'mlnotify-test',
				private_key_id: '2064e75fa13f2366e99762b77a1eae585b6a1be9',
				private_key:
					'-----BEGIN PRIVATE KEY-----\\njZwBzbUMl6+Xu3Ur/SDm0sAg\n-----END PRIVATE KEY-----\n',
				client_email: 'firebase-adminsdk-qmsv3@mlnotify-test.iam.gserviceaccount.com',
				client_id: '115608594952508182814',
				auth_uri: 'https://accounts.google.com/o/oauth2/auth',
				token_uri: 'https://oauth2.googleapis.com/token',
				auth_provider_x509_cert_url: 'https://www.googleapis.com/oauth2/v1/certs',
				client_x509_cert_url:
					'https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-qmsv3%40mlnotify-test.iam.gserviceaccount.com',
			}),
		},
		'mlnotify-register-client-lambda',
	)
	await admin.messaging().subscribeToTopic(registrationToken, topicId)

	// Success
	return {
		statusCode: 200,
		body: JSON.stringify({ message: 'OK' }),
	}
}

export { handler }
