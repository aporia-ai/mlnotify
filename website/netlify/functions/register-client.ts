...
const handler: Handler = async (event) => {
  ...
  try {
    const body: { registrationToken?: string; topicId?: string } = JSON.parse(
      event.body
    );
    if (!body.registrationToken || !body.topicId)
    ...
    return {
      ...
      body: JSON.stringify({
        message:
          "Invalid input. Body must be of type { registrationToken: string, topicId: string }",
        error: e.message,
      }),
    };
  }
  ...
  admin.initializeApp({
    credential: admin.credential.cert({
      ...
      client_email:
        "firebase-adminsdk-qmsv3@mlnotify-test.iam.gserviceaccount.com",
